...
Sparse_DFM <- function(X, r, q = 0, alphas = logspace(-2,3,100), alg = 'EM-sparse', err = 'AR1', standardize = TRUE, max_iter=500, threshold=1e-4) {
  
  
  ##  Sparse DFM with IID or AR(1) idiosyncratic errors and VAR(1) factors:
  ##
  ##  X_t = Lambda.tilde*F.tilde_t + eta_t,  eta_t ~ N(0,Sigma.eta)
  ##  F.tilde_t = A.tilde*F.tilde_{t-1} + u.tilde_t,  u.tilde_t ~ N(0,Sigma.u.tilde)
  ##
  ##  Latent state: F.tilde_t = [F_t e_t]^T for r factors F_t and p idio errors e_t.
  ##
  ##  Loadings: Lambda.tilde = [Lambda I_p] where Lambda are the loadings assumed to be sparse. 
  ...
  if(alg == 'PCA'){       # PCA algorithm applied 
    ...
    ## Kalman Filter and Smoother (Doz (2011) 2-step) 
      ...
      KFS <- kalmanCpp(X, a0_0, P0_0, A.tilde, Lambda.tilde, Sigma.eta, Sigma.u.tilde)
        ...
        state.KF = as.matrix(KFS$factors.KF[2:(n+1),])
        covariance.KF = KFS$covariance.KF[,,2:(n+1)] 
        state.KS = as.matrix(KFS$factors.KS[2:(n+1),])
    ...
    ## EM Algorithm
      ...
      #best.KFS <- kalman(X, a0_0, P0_0, A.tilde, Lambda.tilde, Sigma.eta, Sigma.u.tilde)
      ...
      ## Loop over alphas and calculate BIC until column of Lambda becomes 0 
        ...
        for(alphas.index in 1:length(alphas)) {
          ...
          EM.fit <- EM(X, a0_0, P0_0, A.tilde, Lambda.tilde, Sigma.eta, Sigma.u.tilde, 
                       ...
                       max_iter = max_iter, threshold = threshold, q)
          ...
          if(any(colSums(EM.fit$Lambda.tilde[,1:r]) == 0)){
            break
          }
          ...
          # calculate BIC 
          bic[alphas.index] = bic_function(X, KFS$factors.KS[2:(n+1),1:r], Lambda.tilde[,1:r])
