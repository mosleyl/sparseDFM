...
Sparse_DFM <- function(X, r, alphas = logspace(-2,3,100), alg = 'EM-sparse', err = 'AR1', standardize = TRUE, max_iter=500, threshold=1e-4) {
  
  
  ##  Sparse DFM with AR(1) idiosyncratic errors and VAR(1) factors:
  ##
  ...
  if(alg == 'PCA'){       # PCA algorithm applied 
    ...
    ## EM Algorithm
      ...
      state.EM = as.matrix(KFS$factors.KS[2:(n+1),])
      covariance.EM = KFS$covariance.KS[,,2:(n+1)]
