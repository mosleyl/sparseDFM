...
initPCA <- function(X,r,err='AR1') {
  ...
  # Fill in missing data using splines and moving average 
  # Similar approach to Giannone (2008) and nowcasting package by De Valk et al. (2019)
  fillNA = fill_NA(X)
  X.balanced<- fillNA$X
  ...
  if(err == 'AR1'){
    ...
    # P0_0 is (r+p)x(r+p) variance of state at t=0 
      ...
      a0_0 = rep(0, r+p)
      P_F = matrix(corpcor::pseudoinverse(diag(r*r)- kronecker(A,A)) %*% matrix(Sig_u, ncol = 1), r, r)
    ...
    # P0_0 is rxr variance of state at t=0 
      ...
      a0_0 = rep(0, r)
      P0_0 = matrix(corpcor::pseudoinverse(diag(r*r)- kronecker(A,A)) %*% matrix(Sig_u, ncol = 1), r, r)
